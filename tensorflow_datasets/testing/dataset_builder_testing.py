...
def _np_load(file, mmap_mode=None, allow_pickle=False, **kwargs):
  if not hasattr(file, "read") or not isinstance(file, tf.gfile.GFile):
    raise AssertionError(
        "You MUST pass a `tf.gfile.GFile` instance to `np.load`.")
  if allow_pickle:
    raise AssertionError("Unpicling files is forbidden for security reasons.")
  return _ORGINAL_NP_LOAD(file, mmap_mode, allow_pickle, **kwargs)
